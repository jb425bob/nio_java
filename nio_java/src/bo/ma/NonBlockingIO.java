...
public class NonBlockingIO {
	...
	/*					InetAddress addr = InetAddress.getByName(null);
					...
					if(key.isWritable() && !written){
						//String str ="{\"userid\":\"812781237630002383\",\"appid\":\"1283804\",\"tag\":\"TAG_BW_cdgateway1\",\"channelid\":\"3585936215855423926\"}" ;
						String str = "1234567891234567891234567891";
						/*ByteBuffer header = ByteBuffer.wrap(new byte[]{(byte)0x9d,(byte)0x9d});
						ByteBuffer type = ByteBuffer.wrap(new byte[]{(byte)0x02});
						ByteBuffer length = ByteBuffer.allocate(4);
						length.putInt(str.getBytes().length + 7);
						ByteBuffer content = ByteBuffer.wrap(str.getBytes());
						
						int contentLength = str.getBytes().length + 7;
						ByteBuffer contentBuffer = ByteBuffer.allocate(contentLength);
						contentBuffer.put(new byte[]{(byte)0x9d,(byte)0x9d});
						contentBuffer.put((byte)0x02);
						contentBuffer.putInt(contentLength);
						contentBuffer.put(str.getBytes());
						System.out.println(contentBuffer.toString());;
						System.out.println(contentBuffer.array());
						System.out.println(contentLength);*/
					ByteBuffer contentBuffer = ByteBuffer.allocate(36);
						contentBuffer.put(new byte[]{(byte)0x9d,(byte)0x9d});
						contentBuffer.put((byte)0x01);
						contentBuffer.putInt(36);
						contentBuffer.put((byte)0x01);
						contentBuffer.put(str.getBytes(), 0, 28);
						System.out.println(contentBuffer.toString());;
						System.out.println(contentBuffer.array());
						//System.out.println(contentLength);
					/*	header.flip();
